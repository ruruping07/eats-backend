...
import { Users, UserRole } from 'src/users/entities/users.entity';
...
export class OrderService {
  ...
  async createOrder( customer: Users, { restaurantId, items }: CreateOrderInput, ): Promise<CreateOrderOutput> {
    try {
      ...
      const order = await this.orders.save( this.orders.create({ customer, restaurant, total: orderFinalPrice, items: orderItems, }), );
  ...
  async getOrders( user: Users, { status }: GetOrdersInput, ): Promise<GetOrdersOutput> {
    try {
      let orders: Order[];
  ...
  async getOrder( user: Users, { id: orderId }: GetOrderInput, ): Promise<GetOrderOutput> {
    try {
  ...
  async editOrder( user: Users, { id: orderId, status }: EditOrderInput, ): Promise<EditOrderOutput> {
    try {
      ...
      if (!this.canSeeOrder(user, order)) {
        return { ok: false, error: "Can't see this.", };
      }
      ...
      if (user.role === UserRole.Owner) {
        if (status === OrderStatus.Cooked) {
          await this.pubSub.publish(NEW_COOKED_ORDER, { newOrder, });
        }
      }
  ...
  async takeOrder( driver: Users, { id: orderId }: TakeOrderInput, ): Promise<TakeOrderOutput> {
    try {
      const order = await this.orders.findOne(orderId);
